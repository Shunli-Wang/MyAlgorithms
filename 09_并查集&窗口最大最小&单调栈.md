...
- **实现方法**：考虑无重复值情况，若求左边最近最大、右边最近最大，则需要构建并维持一个由底直顶从大到小的栈，栈内存放元素下标。
...
```java
public static calss windowMax{
    public int L = -1; // [
    public int R = 0; // )
    public LinkedList<Integer> maxToMin;

    static windowMax(){
        maxToMin = new LinkedList<>();
    }

    public static int moveRight(int r, int[] arr){
