KMP算法和Manacher算法都是面试中很多算法的原型，虽然这两个算法自身只是解决了特定的两个问题，但是其衍生工具对很多其他题目有非常高的参考价值。
...
# 4.Manacher算法
- **基本概念**：解决单个字符串中`最长回文子串`长度求解问题，能够将搜索时间控制在O(N)级别。
...
- **重要数组与变量**：
    ...
    - **2.最右回文右边界R**：之前扩的所有位置中，所到达的最右回文右边界位置。此值随着考察点的右移而不断右移。
...
- **4.Manacher算法的情况罗列**：
    - **1.当i位置出现在R外时**：i已经位于R右侧，进行双侧暴力扩，并且一定能够更新R和C。即`--[L---C---R]i--`；
...
<div align=center>
<img src="./img/08_img/ManacherCase2.png" width="520px"/>
</div>
